 ...
 * ============================================
 *
 * Keeps the A786 Satta brand consistent
 * across all public chart pages.
 *
 * No database/result logic is changed.
 */
export async function generateMetadata({
  ...
  let game: any = await Game.findOne({
    slug: slugParam,
    isActive: true,
  }).lean()
   ...
   * don't create an indexable metadata page.
   */
  if (!game) {
    return {
      title: `${BRAND_NAME} – Results & Charts`,
      description:
        'Check latest results and historical charts on A786 Satta.',
      robots: {
        index: false,
        follow: true,
      },
  ...
  /*
   * IMPORTANT:
   * Keep the SEO title focused on the brand.
   *
   * We intentionally do NOT use the old
   * "Satta King" wording in the metadata.
   */
  const title = `Latest Chart & Results | ${BRAND_NAME}`

  const description =
    `Check latest chart updates, recent results and historical data on ${BRAND_NAME}.`
  ...
  return {
    ...
    alternates: {
      canonical: `${SITE_URL}/chart/${slugParam}`,
    },
...
export default async function GameChartPage({
  ...
  /*
   * 2. Try _id if it looks like MongoDB ObjectId
   */
  if (!game && isObjectId(slugParam)) {
    game = await Game.findById(slugParam).lean()
  }
  ...
  /*
   * 3. Case-insensitive slug fallback
   */
  if (!game) {
    const escaped = slugParam.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
  ...
  /*
   * Fetch charts and recent published results
   *
  ...
  const gameName = String(game.name || '').toUpperCase()

  return (
    <div
      className="min-h-dvh grid-bg"
    ...
    >
      ...
      <main style={{ width: '100%' }}>
        <div
          className="px-3 md:px-8 pb-10 w-full"
          style={{
            maxWidth: 1200,
            margin: '0 auto',
          }}
            ...
            <div className="mb-5">
              ...
              <div className="flex gap-2 overflow-x-auto pb-2">
                {(recentResults as any[]).map((r) => (
                  <div
                    key={r._id}
                    className="shrink-0 text-center sk-card px-3 py-2"
                    style={{
                      minWidth: 56,
                    }}
                  >
