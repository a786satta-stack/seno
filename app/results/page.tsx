...
export default async function ResultsPage() {
  ...
  const resultMap = Object.fromEntries(
    (todayResults as any[]).map((r) => [r.gameSlug, r])
  )

  // ✅ FIXED: IST display date
  ...
  return (
    <div className="min-h-dvh grid-bg" style={{ background: '#FFFFFF' }}>
      <Header
        tickerItems={(games as any[]).map((g: any) => ({
          gameName: g.name,
          resultNumber: resultMap[g.slug]?.resultNumber ?? '--',
        }))}
      />
      ...
      <main style={{ width: '100%' }}>
        ...
        >
            ...
            <div>
              {(games as any[]).map((game) => {
                ...
                return (
                  <div
                    key={game._id}
                    className="flex items-center px-4 py-4 border-b border-yellow-100"
          ...
          <div
            className="mt-5 rounded-xl px-4 py-3 font-mono text-xs"
            style={{
              background: '#FFFDE7',
